...
← 返回 [wiki 总览](index.md) · 相关页：[agent持续学习](agent持续学习.md)（学习闭环）、[harness](harness.md)（工程载体）、[agent自进化](agent自进化.md)（记忆作为可进化面）
...
5. **评测缺口**：尚无面向 coding harness 的记忆基准（本项目论文机会，见 search tree A-5/C-6）
